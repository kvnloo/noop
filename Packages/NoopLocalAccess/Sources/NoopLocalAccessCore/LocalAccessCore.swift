...
public struct RespBucketRow: Equatable, Sendable {
    public let ts: Int
    public let raw: Double
}
...
public struct GravityBucketRow: Equatable, Sendable {
    public let ts: Int
    public let x: Double
    public let y: Double
    public let z: Double
...
public struct BatteryBucketRow: Equatable, Sendable {
    public let ts: Int
    public let soc: Double
    public let mv: Double
}

public struct SleepStateBucketRow: Equatable, Sendable {
    public let ts: Int
...
public final class ReadonlyNoopStore {
    ...
    public func latestBatteryTs(deviceId: String) throws -> Int? {
        guard tableNames.contains("battery") else { return nil }
        return try dbQueue.read { db in
            try Int.fetchOne(db, sql: "SELECT MAX(ts) FROM battery WHERE deviceId = ?", arguments: [deviceId])
        }
    }
    ...
    public func respBuckets(deviceId: String, from: Int, to: Int, bucketSeconds: Int) throws -> [RespBucketRow] {
        guard tableNames.contains("respSample") else { return [] }
        let bucket = max(1, bucketSeconds)
        return try dbQueue.read { db in
            try Row.fetchAll(db, sql: """
                SELECT (ts / ?) * ? AS bucket, AVG(raw) AS avgRaw
    ...
    public func gravityBuckets(deviceId: String, from: Int, to: Int, bucketSeconds: Int) throws -> [GravityBucketRow] {
        guard tableNames.contains("gravitySample") else { return [] }
        let bucket = max(1, bucketSeconds)
        return try dbQueue.read { db in
            try Row.fetchAll(db, sql: """
                SELECT (ts / ?) * ? AS bucket, AVG(x) AS avgX, AVG(y) AS avgY, AVG(z) AS avgZ
                FROM gravitySample
                WHERE deviceId = ? AND ts >= ? AND ts <= ?
                GROUP BY ts / ?
                ORDER BY bucket ASC
                """, arguments: [bucket, bucket, deviceId, from, to, bucket]).map {
                GravityBucketRow(ts: $0["bucket"], x: $0["avgX"], y: $0["avgY"], z: $0["avgZ"])
            }
        }
    }

    public func batteryBuckets(deviceId: String, from: Int, to: Int, bucketSeconds: Int) throws -> [BatteryBucketRow] {
        guard tableNames.contains("battery") else { return [] }
        let bucket = max(1, bucketSeconds)
        return try dbQueue.read { db in
            try Row.fetchAll(db, sql: """
                SELECT (ts / ?) * ? AS bucket, AVG(soc) AS avgSoc, AVG(mv) AS avgMv
                FROM battery
                WHERE deviceId = ? AND ts >= ? AND ts <= ?
                GROUP BY ts / ?
                ORDER BY bucket ASC
                """, arguments: [bucket, bucket, deviceId, from, to, bucket]).map {
                BatteryBucketRow(ts: $0["bucket"], soc: $0["avgSoc"], mv: $0["avgMv"])
            }
        }
    }

    public func sleepStateBuckets(deviceId: String, from: Int, to: Int, bucketSeconds: Int) throws -> [SleepStateBucketRow] {
        guard tableNames.contains("sleepStateSample") else { return [] }
        let bucket = max(1, bucketSeconds)
        return try dbQueue.read { db in
            try Row.fetchAll(db, sql: """
                SELECT (ts / ?) * ? AS bucket, AVG(state) AS avgState
                FROM sleepStateSample
                WHERE deviceId = ? AND ts >= ? AND ts <= ?
                GROUP BY ts / ?
                ORDER BY bucket ASC
                """, arguments: [bucket, bucket, deviceId, from, to, bucket]).map {
                SleepStateBucketRow(ts: $0["bucket"], state: $0["avgState"])
            }
        }
    }
...
public final class NoopDataAccess {
    ...
    public func respSeries(
        ...
        let resolvedDeviceId = overrideDeviceId ?? deviceId
        let buckets = try store.respBuckets(
            deviceId: resolvedDeviceId,
            from: fromTs,
            to: toTs,
            bucketSeconds: bucketSeconds
    ...
    public func gravitySeries(
        ...
        let resolvedDeviceId = overrideDeviceId ?? deviceId
        let buckets = try store.gravityBuckets(
            deviceId: resolvedDeviceId,
            from: fromTs,
            to: toTs,
            bucketSeconds: bucketSeconds
    ...
    public func batterySeries(
        ...
        let resolvedDeviceId = overrideDeviceId ?? deviceId
        let buckets = try store.batteryBuckets(
            deviceId: resolvedDeviceId,
            from: fromTs,
            to: toTs,
            bucketSeconds: bucketSeconds
    ...
    public func sleepStateSeries(
        ...
        let resolvedDeviceId = overrideDeviceId ?? deviceId
        let buckets = try store.sleepStateBuckets(
            deviceId: resolvedDeviceId,
            from: fromTs,
            to: toTs,
            bucketSeconds: bucketSeconds
